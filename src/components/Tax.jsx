import { images } from "../data/images";
import { CALENDAR_URL } from "../config";

export default function Tax() {
  return (
    <section className="tax">
      <div className="taxImage"><img src={images.taxBook} alt="Direito Tributário" /></div>
      <div className="taxText">
        <p className="eyebrow">Especialização</p>
        <h2>Fiscalidade Internacional.</h2>
        <p>Acompanhamento estratégico para quem vive, trabalha, investe ou possui património entre Portugal e a Noruega.</p>
        <ul>
          <li>Planeamento fiscal internacional</li>
          <li>Residência fiscal</li>
          <li>Dupla tributação</li>
          <li>Rendimentos e património transfronteiriço</li>
        </ul>
        <a className="button gold" href={CALENDAR_URL} target="_blank" rel="noreferrer">Agendar consulta</a>
      </div>
    </section>
  );
}
